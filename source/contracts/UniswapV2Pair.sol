...
contract UniswapV2Pair is IUniswapV2Pair, UniswapV2ERC20 {
    ...
    //最小流动性 = 1000, 1e-15, 最小份额1e-18的1000倍
    ...
    function swap(
        ...
        //获取`储备量0`,`储备量1`
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        //确认`输出数量0,1` < `储备量0,1`, 输出数量需小于储备量, 因初始创建流动性时,附加了初始流动性,1e-15,因而必然取不完
        require(
        ...
        // 公式 balance0 > _reserve0 - amount0Out 确认了交易后池中的代币数量是否减少了。
        // 如果 balance0 大于 _reserve0 - amount0Out，则表示有输入的代币，因为当前余额大于预期应有的余额。因而输入量就等于差值.,否则没有输入量
        ...
        );
        {
            ...
            //调整后的余额0 = 余额0 * 1000 - (amount0In * 3), 3为0.3%的手续费
            uint256 balance0Adjusted = balance0.mul(1000).sub(amount0In.mul(3));
            //调整后的余额1 = 余额1 * 1000 - (amount1In * 3)
            uint256 balance1Adjusted = balance1.mul(1000).sub(amount1In.mul(3));
            //确认balance0Adjusted * balance1Adjusted >= 储备0 * 储备1 * 1000000, 调整后的k值需大于等于原始的k值, 因为收取了手续费
            // 通过这种方式，可以确保在考虑手续费后，交易前后池子的平衡得到维护。
            require(
